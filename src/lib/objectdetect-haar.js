// TAKEN from: https://github.com/thechriswalker/haar-object-detection/blob/master/packages/haar-detect/index.js
/*
 *  Source: https://raw.githubusercontent.com/mtschirs/js-objectdetect/55b5c16f5aa61c569d0d36e5ff43e0f190ec015f/js/js
 *  Modifications:
 *      - added wrapper function as only export
 *      - switched `*` method calls to local var refs
 *      - remove execessive exports from IIFE (allows minimizing to remove the unused functions)
 *      - run through `prettier` (because all my code is...)
 *  Motivation: commonjs compatibility, removal of included classifiers
 */
/**
...
var objectdetect = (function() {
    ...
    convertRgbaToGrayscale = function(src, dst) {
        ...
        computeCanny = function(src, srcWidth, srcHeight, dst) {
            ...
            for (var x = 0; x < srcWidth; ++x) {
                ...
                for (var y = 2; y < srcHeight - 2; ++y) {
                    index += srcWidth;
                    buffer2[index] = 0.1117 *
                        buffer1[index - srcWidth - srcWidth] +
        ...
        computeSat = function(src, srcWidth, srcHeight, dst) {
            ...
            if (!dst)
                dst = new Uint32Array(
                    srcWidth * srcHeight + dstWidth + srcHeight
                );
        ...
        equalizeHistogram = function(src, step, dst) {
            ...
            var norm = 255 * step / srcLength, prev = 0;
            for (var i = 0; i < 256; ++i) {
                var h = hist[i];
                prev = (h += prev);
        ...
        compileClassifier = function(src, width, scale, dst) {
            ...
            for (var srcIndex = 1, iEnd = src.length - 1; srcIndex < iEnd; ) {
                ...
                var numComplexClassifiers = (dstUint32[++dstIndex] = src[
                    ++srcIndex
                ]);
                for (var j = 0, jEnd = numComplexClassifiers; j < jEnd; ++j) {
                    var tilted = (dst[++dstIndex] = src[++srcIndex]);
                    var numFeaturesTimes3 = (dstUint32[++dstIndex] = src[
                        ++srcIndex
                    ] * 3);
                    if (tilted) {
                        for (
                            var kEnd = dstIndex + numFeaturesTimes3;
                            dstIndex < kEnd;
            ...
            for (var x = 0; x + windowWidth < width; x += step) {
                ...
                for (var y = 0; y + windowHeight < height; y += step) {
                    ...
                    for (var i = 1, iEnd = classifier.length - 1; i < iEnd; ) {
                        ...
                        for (
                            var j = 0, jEnd = classifierUint32[++i];
                            j < jEnd;
                            ++j
                        ) {
        ...
        groupRectangles = function(rects, minNeighbors, confluence) {
            ...
            for (var i = 0; i < rectsLength; ++i) {
                var rect = rects[i], group = groups[labels[i]];
                group[0] += rect[0];
                group[1] += rect[1];
                group[2] += rect[2];
                group[3] += rect[3];
    ...
    var detector = (function() {
        ...
        detector.prototype.detect = function(
            ...
            return (group
                ? groupRectangles(rects, group)
                : rects).sort(function(r1, r2) {
                return r2[4] - r1[4];
            });
        };
...
// wrapper added
module.exports = (exports = function createDetector(classifier, options) {
    var opts = Object.assign(options || {}, {
        width: 640,
        height: 480,
        scaleFactor: 1.2
    });
    var detector = new objectdetect.detector(
        opts.width,
        opts.height,
        opts.scaleFactor,
        classifier
